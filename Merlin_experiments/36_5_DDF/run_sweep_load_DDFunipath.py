...
import re
import csv
from topoResearch.pickle_data import pickle_ddf_unipath
...
for traffic in ["uniform", "shift_1", "shift_half"]:
    for routing in ["nonadaptive", "ugal_precise", "ugal_threshold"]:
        ...
        config_dict = {
            ...
            'topo_name':"DDF",
            'paths':"DDFunipath",
            'routing_algo': routing, # same for two subnets
            'traffic_pattern': traffic
            # 'traffic_pattern': "shift_1"
        }

        pickle_ddf_unipath((config_dict['V'], config_dict['D']))
